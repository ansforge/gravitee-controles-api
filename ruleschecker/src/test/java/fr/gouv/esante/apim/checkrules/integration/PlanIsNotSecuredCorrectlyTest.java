...
@SpringBootTest(
        classes = {PlanIsNotSecuredCorrectlyTest.class, CheckRulesService.class,
...
@ActiveProfiles({"test"})
@Slf4j
class PlanIsNotSecuredCorrectlyTest {
    ...
    void testPlanIsNotSecuredCorrectlyTest() throws Exception {
        String expectedMessage = "Aucun plan ne dispose d'un moyen d’authentification et d’identification de l’utilisateur sur cette API";
        ...
        Optional<RuleResult> rule3_1_1 = apiResults.stream().filter(r -> r.getRuleName().equalsIgnoreCase("3.1.1 - Paramétrer au moins un moyen d’authentification et d’identification de l’utilisateur")).findFirst();
